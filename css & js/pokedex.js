...
// ********** Primer ejemplo de un pokedexExpress **********
/*const fetchPokemon = () => { // Consulta a API
    const pokeName = document.getElementById("pokeName");
    let pokeInput = pokeName.value.toLowerCase(); // Convierte a minúsculas
    //pokeInput = pokeName.toLowerCase();
    const url = `https://pokeapi.co/api/v2/pokemon/${pokeInput}`; // URL de la API
    fetch(url).then((res) => { // Fetch de la API
        if (res.status != "200") { // Si no hay resultados (Manejo de errores)
            console.log(res);
            pokeImage("./pokemon-sad.gif")
        }
        else { // Si hay resultados
            return res.json(); // Retorna la respuesta en formato JSON
        }
    }).then((data) => { // Retorno de la API
        if (data) {
            console.log(data); // Imprime la data de la API
            let pokeImg = data.sprites.front_default; // Imagen de la API
            pokeImage(pokeImg);
            console.log(pokeImg);
        }
    });
...
const pokeImage = (url) => { // Función para cambiar la imagen
    const pokePhoto = document.getElementById("pokeImg"); // Selecciona la imagen
    pokePhoto.src = url; // Cambia la imagen
}*/